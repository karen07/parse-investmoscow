...
retry() {
    ...
    while :; do
        ...
        if [ "$i" -ge "$MAX_TRIES" ]; then
            log "ERROR: $label failed after $MAX_TRIES tries"
            return 1
        fi

        log "WARN: $label failed, retry $((i + 1))/$MAX_TRIES"
        sleep $((SLEEP_BASE * i))
        i=$((i + 1))
    done
}
...
if [ -z "$totalCount" ] || [ "$totalCount" = "null" ]; then
    log 'ERROR: failed to extract totalCount'
    exit 1
fi

pages=$(((totalCount + PAGE_SIZE - 1) / PAGE_SIZE))
log "filteredCount=$filteredCount totalCount=$totalCount pageSize=$PAGE_SIZE pages=$pages"
