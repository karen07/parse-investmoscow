...
def summarize_group(name: str, rows: dict):
    ...
    for k in sorted(rows.keys()):
        ...
        print(f"{k};{len(vals)};{avg:.2f};{med:.2f}")
...
def main():
    ...
    for p in files:
        try:
            ...
            area_s = "" if area is None else f"{area:.2f}"

            print(
                f"{tender_id};{year_s};{area_s};{start};{final};{ratio:.6f};"
                f"{delta_pct:.2f};{delta_rub};{near_dist:.2f};{p}"
            )
    ...
    print(f"avg_ratio={sum(ratios)/len(ratios):.6f}")
    print(f"avg_delta_pct={sum(deltas_pct)/len(deltas_pct):.2f}")
    print(f"avg_delta_rub={sum(deltas_rub)/len(deltas_rub):.0f}")
    print(f"median_ratio={statistics.median(ratios):.6f}")
    print(f"median_delta_pct={statistics.median(deltas_pct):.2f}")
